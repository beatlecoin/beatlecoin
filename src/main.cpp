...
uint256 hashGenesisBlock("0x78e29d40eb9ae90768c95950a6e665455d00747dea755a6173ba8ef44eac7552");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // BeatleCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "BeatleCoin Signed Message:\n";
...
{
    // BeatleCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
	...
	if(nHeight < 990196) // stops generation of more coins
	{
		nSubsidy = 1 * COIN;
	}
	if(nHeight < 909)
    ...
    if(nHeight == 1)
    {
       nSubsidy = 10000.79 * COIN;
	}
    if(nHeight == 64) // First extra block, check out "When i'm 64"
    {
       nSubsidy = 0.6 * COIN;
	}
    if(nHeight == 910) // Yeah, the One After 909, 9 coins earned!
    {
       nSubsidy = 9 * COIN;
	}
    if(nHeight == 1963) // 1963, Introducing The Beatles, 3 coins block!
    {
       nSubsidy = 3 * COIN;
	}
    if(nHeight == 1964) // 1964, Beatlemania spreads all over the world, 5 more BeatleCoins!
    {
       nSubsidy = 5 * COIN;
	}
    if(nHeight == 1965) // 1965, "Help!" and "Rubber Soul" released, 10 coins for this block.
    {
       nSubsidy = 10 * COIN;
	}
    if(nHeight == 1966) // 1966, The last show but not the last record! 15 coins reward.
    {
       nSubsidy = 15 * COIN;
	}
    if(nHeight == 1967) // 1967, All you need is strawberry fields and 20 extra coins!
    {
       nSubsidy = 20 * COIN;
	}
    if(nHeight == 1968) // 1968, White Album released, 25 coins earned!
    {
       nSubsidy = 25 * COIN;
	}
    if(nHeight == 1969) // 1969, Abbey Road sessions, plus 30 coins.
    {
       nSubsidy = 30 * COIN;
	}
    if(nHeight == 1970) // Block 1970, Let It Be 35 coins.
     {
       nSubsidy = 35 * COIN;
	}
    if(nHeight == 1995) // Together again, releasing Free as a bird. +10 Coins!
     {
       nSubsidy = 10 * COIN;
        }
    if(nHeight == 4000) // "Four thousand holes in Blackburn, Lancashire..." 10X Block! 
     {
       nSubsidy = 10 * COIN;
        }
    if(nHeight == 9999) // Number nine...Number nine...Number nine...Number nine... 10X Block! 
     {
       nSubsidy = 10 * COIN;
        }    
    if(nHeight == 49537)
     {
       nSubsidy = 20 * COIN; // 20 extra coins!
        }    
    if(nHeight == 99999) // Number nine...Number nine...Number nine...Number nine...Number nine... 10X Block! 
     {
       nSubsidy = 10 * COIN;
        }
	if(nHeight >= 990196) // stops generation of more coins
...
static const int64 nTargetTimespan = 480; // BeatleCoin: 8 minutes
static const int64 nTargetSpacing = 64; // BeatleCoin: 64 seconds
static const int64 nInterval = nTargetTimespan / nTargetSpacing;
static const int64 nTargetTimespanRe = 960; // BeatleCoin: 16 Minutes
static const int64 nTargetSpacingRe = 64; // BeatleCoin: 64 seconds
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // BeatleCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // BeatleCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // BeatleCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xf0ad38072924b0f98075471f8c6cb30de3a3e0e51f9d36fe15f629492ffb07da");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Kiev Protesters Set Square Ablaze to Thwart Police";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1394115224;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 3481105;

        if (fTestNet)
        {
            block.nTime    = 1393442510;
            block.nNonce   = 753724;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x8f71932cb253bcba672cd8eb8bfc14724f7eb76cc7fbb12e6392df62260330d9"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // BeatleCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BeatleCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BeatleCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BeatleCoinMiner(CWallet *pwallet)
{
    printf("BeatleCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("beatlecoin-miner");
    ...
    try { loop {
        ...
        printf("Running BeatleCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BeatleCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BeatleCoinMiner, pwallet));
}
